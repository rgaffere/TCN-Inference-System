...
def load_json_weights(model, weights_path):
    ...
    model.input_layer.conv2.conv.weight.data = torch.tensor(w["inputFilter2"], dtype=torch.float32).unsqueeze(1)
    ...
    for i, layer in enumerate(model.hidden_layers):
        layer.conv1.conv.weight.data = torch.tensor(w["hiddenFilter1"][i], dtype=torch.float32).unsqueeze(1)
        layer.conv1.conv.bias.data = torch.tensor(w["hiddenBias1"][i], dtype=torch.float32)

        layer.conv2.conv.weight.data = torch.tensor(w["hiddenFilter2"][i], dtype=torch.float32).unsqueeze(1)
