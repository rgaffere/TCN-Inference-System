...
module resi_block_mixer_tb;
    ...
    typedef logic signed [W_BIT_WIDTH-1:0] wmat_t  [0:NUM_CHANNELS-1][0:NUM_CHANNELS-1][0:KERNEL_LEN-1];
    ...
    initial begin
        if ($test$plusargs("VCD")) begin
            $dumpfile("resi_block_mixer_tb.vcd");
            $dumpvars(0, resi_block_mixer_tb);
        end
    end
    ...
    task automatic clear_weights;
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            ...
            for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                for (int k = 0; k < KERNEL_LEN; k++) begin
                    weights1[out_ch][in_ch][k] = '0;
                    weights2[out_ch][in_ch][k] = '0;
                end
            end
        end
    endtask
    ...
    task automatic build_expected(input int id, input chvec_t v);
        ...
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            ...
            for (int k = 0; k < KERNEL_LEN; k++) begin
                ...
                if (hist_id >= 0) begin
                    for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                        acc += int'($signed(tok_in[hist_id][in_ch])) *
                               int'($signed(weights1[out_ch][in_ch][k]));
                    end
                end
            end

            tok_q1[id][out_ch] = relu_quant(acc);
        end
        ...
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            ...
            for (int k = 0; k < KERNEL_LEN; k++) begin
                ...
                if (hist_id >= 0) begin
                    for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                        acc += int'($signed(tok_q1[hist_id][in_ch])) *
                               int'($signed(weights2[out_ch][in_ch][k]));
                    end
                end
            end

            tok_q2[id][out_ch] = relu_quant(acc);
            tok_exp[id][out_ch] = sat8(int'($signed(tok_q2[id][out_ch])) +
                                       int'($signed(tok_in[id][out_ch])));
        end
    endtask
    ...
    task automatic set_mixed_weights;
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                weights1[out_ch][in_ch][0] = s8(20 + ((out_ch + in_ch) % 7));
                weights1[out_ch][in_ch][1] = s8(-12 + ((out_ch + in_ch) % 5));
                weights1[out_ch][in_ch][2] = s8(7 - ((out_ch + in_ch) % 6));

                weights2[out_ch][in_ch][0] = s8(16 - ((out_ch + in_ch) % 4));
                weights2[out_ch][in_ch][1] = s8(-10 + ((out_ch + in_ch) % 3));
                weights2[out_ch][in_ch][2] = s8(9 + ((out_ch + in_ch) % 5));
            end

            bias1[out_ch] = (out_ch % 2) ? -32'sd384 : 32'sd768 + out_ch;
            bias2[out_ch] = (out_ch % 3) ?  32'sd512 : -32'sd256;
        end
    endtask
        ...
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                for (int k = 0; k < KERNEL_LEN; k++) begin
                    ...
                    weights1[out_ch][in_ch][k] = s8(x);

                    x = $urandom_range(0, 63) - 32;
                    weights2[out_ch][in_ch][k] = s8(x);
                end
            end

            bias1[out_ch] = $urandom_range(0, 4095) - 2048;
            bias2[out_ch] = $urandom_range(0, 4095) - 2048;
        end
    endtask
    ...
    task automatic set_saturation_weights;
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                weights1[out_ch][in_ch][0] = 8'sd127;
                weights1[out_ch][in_ch][1] = 8'sd64;
                weights1[out_ch][in_ch][2] = 8'sd32;

                weights2[out_ch][in_ch][0] = 8'sd127;
                weights2[out_ch][in_ch][1] = 8'sd127;
                weights2[out_ch][in_ch][2] = 8'sd127;
            end

            bias1[out_ch] = 32'sd32767;
            bias2[out_ch] = 32'sd32767;
        end
    endtask
    ...
    task automatic set_tap_isolation(input int tap);
        for (int out_ch = 0; out_ch < NUM_CHANNELS; out_ch++) begin
            ...
            for (int in_ch = 0; in_ch < NUM_CHANNELS; in_ch++) begin
                for (int k = 0; k < KERNEL_LEN; k++) begin
                    weights1[out_ch][in_ch][k] =
                        ((out_ch == in_ch) && (k == tap)) ? 8'sd64 : 8'sd0;
                    weights2[out_ch][in_ch][k] =
                        ((out_ch == in_ch) && (k == 0)) ? 8'sd64 : 8'sd0;
                end
            end
        end
    endtask
    ...
    initial begin
        ...
        $display("\n=== resi_block_mixer cadence-locked bit-exact TB ===");
